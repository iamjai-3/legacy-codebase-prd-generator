...
from src.agentic.message_history import ToolUse

if TYPE_CHECKING:
    from src.agentic.agentic_base import AgenticAgent
...
class AgentRunner:
        ...
        for self.iteration in range(config.max_iterations):
            ...
            try:
                ...
                if response.stop_reason == "end_turn":
                    ...
                    if text_content:
                        self.logger.info(
                            f"Agent completed in {self.iteration + 1} iterations"
                        )
                        return text_content
    ...
    async def _call_claude(self) -> Any:
        ...
        if estimated_tokens > max_context_tokens:
            self.logger.warning(
                f"Context too large ({estimated_tokens} tokens). Truncating..."
            )
