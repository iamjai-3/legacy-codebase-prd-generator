...
def get_files_info(working_directory: str, directory: str = ".") -> str:
    ...
    """
    is_valid, target_dir, error = _validate_path(working_directory, directory)
    ...
    try:
        ...
        for item in sorted(target_dir.iterdir()):
            try:
                ...
                files_info.append(f"- {item.name}: Error reading file info: {e}")
...
def find_files(
    ...
    """
    is_valid, search_dir, error = _validate_path(working_directory, directory)
    ...
    try:
        ...
        for match in search_dir.glob(pattern):
            ...
            try:
                ...
                matches.append(
                    f"- {rel_path}: file_size={size} bytes, is_dir={is_dir}"
                )
...
def create_directory(working_directory: str, directory: str) -> str:
    ...
    Returns:
        Success message or error message
    """
    is_valid, full_path, error = _validate_path(working_directory, directory)
    
    if not is_valid:
        return f"Error: Cannot create '{directory}' as it is outside the permitted working directory"
